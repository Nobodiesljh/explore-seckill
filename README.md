...
- [初探并发编程：秒杀系统](#-----------)
  * [一、最原始网页](#-------)
    ...
    + [3. 创建数据库/创建项目工程](#3-------------)
    + [4. 搭建初始商品购买网页](#4-----------)
    + [5. 第一次JMeter压测：仅测试访问商品的信息页面](#5----jmeter---------------)
  * [二、商品信息静态数据优化](#------------)
    + [1. Redis缓存 feature/staticResImpro-Redis分支](#1-redis---feature-staticresimpro-redis--)
    + [2. 第二次访问商品信息页面JMeter压测：加入Redis缓存后](#2------------jmeter-----redis---)
    + [3. 静态化处理等](#3-------)
  * [三、(单机)秒杀系统 feature/seckillNotDistribute分支](#-----------feature-seckillnotdistribute--)
    + [0. 准备](#0---)
    + [1. case1:不加锁,出现超卖现象](#1-case1-----------)
    + [2. case2:加ReentrantLock,秒杀正常](#2-case2--reentrantlock-----)
    + [3. case3:自定义注解+AOP,正常](#3-case3-------aop---)
    + [4. case4:数据库悲观锁(查询加锁),正常](#4-case4----------------)
    + [5. case5:数据库悲观锁(更新加锁),正常](#5-case5----------------)
    + [6. case6:数据库乐观锁，正常](#6-case6----------)
    + [7. case7:JDK内置阻塞队列，正常](#7-case7-jdk---------)
  * [四、分布式秒杀系统 feature/seckillDistribute分支](#----------feature-seckilldistribute--)
    + [0. 准备](#0----1)
    + [1. case1:基于Redisson的Redis分布式锁，正常](#1-case1---redisson-redis-------)
    + [2. case2:基于Curator的Zookeeper分布式锁，正常](#2-case2---curator-zookeeper-------)
    + [3. case3:Redis的List队列，正常](#3-case3-redis-list-----)
    + [4. case4:Redis原子递减,正常](#4-case4-redis-------)
    + [5. 其他](#5---)
  * [五、秒杀系统性能提升](#----------)
